...
import "deploy/foundry/deployments/DeployOffchainOracleOptimism.s.sol";

contract OffchainOracleOptimismTest is Test {
    DeployOffchainOracleOptimism public deploy;
    OffchainOracle public offchainOracle;
    MultiWrapper public multiWrapper;
    IOracle[] public oracles;

    function setUp() public {
        vm.createSelectFork("optimism", 126389132);
        deploy = new DeployOffchainOracleOptimism();
        deploy.setUp();
        deploy.run();
    ...
    function testGetRate() public view {
        ...
        IERC20 dstToken = IERC20(0x7F5c764cBc14f9669B88837ca1490cCa17c31607); //usdc bridged

        uint256 weightedRate = offchainOracle.getRate({srcToken: srcToken, dstToken: dstToken, useWrappers: false});

        assertEq(weightedRate, 2432125683);
    }
}
